...
do
  if [ "$(ls -A /tmp/output-$1/testcase.sorted/$i)" ]; then
    for tc_file in /tmp/output-$1/testcase.sorted/$i/*
    do
      echo $tc_file
      cp $tc_file ./$4
  ...
  if [ "$(ls -A /tmp/output-$1/crash.sorted/$i)" ]; then
    for crash_file in /tmp/output-$1/crash.sorted/$i/*
    do
      echo $crash_file
      cp $crash_file ./$4
      timeout -k 3 10 ./qemu-trace-bbcount-x64 ./$1 $3 > /dev/null 2>&1
...
if [ "$(ls -A /tmp/output-$1/crash.renamed/)" ]; then
  ...
  for crash_file in /tmp/output-$1/crash.renamed/*
  do
    echo $crash_file
    cp $crash_file ./$4
    rm -f ./core
...
if [ "$(ls -A /tmp/output-$1/crash.reproduced/)" ]; then
  ...
  for crash_file in /tmp/output-$1/crash.reproduced/*
  do
    echo $crash_file
    cp $crash_file ./$4
    timeout -k 3 10 ./$1 $3
