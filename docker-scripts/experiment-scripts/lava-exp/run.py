...
def run_eclipser(program, cmdline, inputfile, timelimit):
    ...
    if not os.path.isdir('/tmp/output-%s' % program):
        exit(1)
    # Replay test cases
    cmd = './replay_eclipser.sh %s "%s" %s > /tmp/log-replay 2>&1' % \
            (program, cmdline, inputfile)
    os.system(cmd)
...
def run_aflfast (program, cmdline, inputfile, timelimit):
    ...
    if not os.path.isdir('/tmp/output-%s' % program):
        exit(1)
    # Replay test cases
    cmd = './replay_aflfast.sh %s "%s" %s > /tmp/log-replay 2>&1' % \
            (program, cmdline, inputfile)
    os.system(cmd)
    # Export results to volume
...
def run_lafintel (program, cmdline, inputfile, timelimit):
    ...
    if not os.path.isdir('/tmp/output-%s' % program):
        exit(1)
    # Replay test cases
    cmd = './replay_lafintel.sh %s "%s" %s > /tmp/log-replay 2>&1' % \
            (program, cmdline, inputfile)
    os.system(cmd)
    # Export results to volume
